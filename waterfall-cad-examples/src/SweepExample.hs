...
import Linear ( V3 (..), (*^), angle, unit, _x, zero, V2 )

sweepExample :: Solid
sweepExample = 

    let sweepPath = Path.pathFrom (zero :: V3 Double)
        ...
        sweepProfile = Shape.fromPath $
                ...
                Path2D.bezier (0.25 *^ unit _x :: V2 Double) (0.5 *^ unit _x) (0.5 *^ angle (pi/6)) (0.25 *^ angle (pi/6))
